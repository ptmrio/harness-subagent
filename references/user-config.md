...
`code-review-visual`: do **not** put “forbid a browser stack” in the brief. Name Playwright (or the parent’s shots). Codex: `spawn.sh --mode visual` and `--image` per screenshot. Claude/Grok: `--mode visual` runs as a read-only review; shots via `--add-dir` / named paths.
